...
function IG_BP_Inversion(
    ...
    # results = LeastSquaresOptim.optimize(J_λ, λ0, LevenbergMarquardt(LeastSquaresOptim.QR());
    #                                     autodiff=:forward, 
    #                                     iterations = 50,
    #                                     lower = lower, upper = upper)

    # Γ_opt, Δ_opt = results.minimizer
    ...
    # bounds = softplus_inv.((1, τ_max))
    # results = bboptimize(J_z; 
    #                      SearchRange = [bounds, bounds], 
    #                      NumDimensions = 2, MaxTime = 150, 
    #                      MaxSteps=5000)
    # Γ_opt, Δ_opt = best_candidate(results)

    
    # results = bboptimize(λ-> J(λ, t_obs; τ_max=τ_max, integr=integr, C0=C0), u0; 
    #                      SearchRange = [(1, τ_max), (1, τ_max)], 
    #                      NumDimensions = 2, MaxTime = 150, 
    #                      MaxSteps=5000)
    # Γ_opt, Δ_opt = best_candidate(results)
