...
begin 
	...
	Δt = 5
	t = Float64.(collect(1:Δt:200)) #define time of integration
...
begin
	...
	t_obs = t[obs_idx]; 
	σ = 0.1; obs_error = rand(Normal(0, σ), nobs)
...
begin
	...
	function MaxEntFuncDenominator(λ::AbstractVector{T}; a=0, b=10000, N=200)  where T
	    ...
	    return quadgk(τ -> MaxEntFuncNumerator(τ, λ, t_obs), 0.0, τ_max)[1]
	end
	...
	function J2_jac!(J2_jac, λ; a=0, b=10000, N=100)
		...
		jac_vec = [quadgk(τ -> C_s_vec[i](τ) * MED(τ) * (Ĉi[i] - C_s_vec[i](τ)), 0, τ_max)[1] for i in 1:nC]
	    J2_jac .= jac_vec
	end

	results2 = 	nlsolve(J2!, J2_jac!, 1 .+ zero(y_obs)).zero
